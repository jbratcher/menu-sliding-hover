...
const createAnimatedNav = (elementId = null, documentBaseFontSize = 16) => {
  ...
  const initialXDistanceInPx =
    links[0].getBoundingClientRect().x -
    navElement.getBoundingClientRect().x -
    padding / 2;

  const initialYDistanceInPx =
    links[0].getBoundingClientRect().y -
    navElement.getBoundingClientRect().y -
    padding / 3;
  ...
  links.forEach((link) => {
    link.addEventListener("mouseenter", () => {
      ...
      let xDistanceFromStart = pxToRem(
        link.getBoundingClientRect().x -
          links[0].getBoundingClientRect().x +
          initialXDistanceInPx
      );

      let yDistanceFromStart = pxToRem(
        link.getBoundingClientRect().y -
          links[0].getBoundingClientRect().y +
          initialYDistanceInPx
      );

      // update hover position x CSS variable to match hovered link location
      navElement.style.setProperty("--hover-position-x", xDistanceFromStart);

      // update hover position x CSS variable to match hovered link location
      navElement.style.setProperty("--hover-position-y", yDistanceFromStart);
